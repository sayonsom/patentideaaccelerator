...
export function FMEAInversion({ data, onChange }: FMEAInversionProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-display font-bold text-text-primary mb-1">FMEA Inversion</h2>
        ...
        {entries.map((entry, i) => (
          <Card key={entry.id}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-text-primary">Failure #{i + 1}</h3>
              <div className="flex items-center gap-2">
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    ...
                    className="rounded border-border-default bg-surface-deep text-accent-gold focus:ring-accent-gold/40"
                  />
                  <span className="text-xs text-accent-gold font-medium">Patent candidate</span>
                </label>
                {entries.length > 1 && (
                  <button
                    type="button"
